...
def export_data_to_file(data: dict) -> None:
    with xw.App(visible=False) as app:
        wb = app.books.open(r'egl_booking_handler\tests\EGL_VOYAGE_NOT_EXISTING.xlsx')
        sheet = wb.sheets['Data']
        row = sheet.range('A1').current_region.last_cell.row + 1
        last_row = 'A' + str(row)

        data_dict = [{'navis_voy': data['navis_voy'],
                     'booking_no': data['booking_no'],
                     'tod': data['tod'],
                     'count_40hc': data['count_40hc'],
                     'weight_40hc': data['weight_40hc'],
                     'count_40dv': data['count_40dv'],
                     'weight_40dv': data['weight_40dv'],
                     'count_20dv': data['count_20dv'],
                     'weight_20dv': data['weight_20dv'],
                     }]

        df = pd.DataFrame().from_dict(data_dict)
        sheet.range(last_row).options(pd.DataFrame,
                                                 index=False,
                                                 header=False).value = df
        wb.save()
        wb.close()
...
def update_equ_info(data: dict, bool_dict: dict) -> None:
    ...
    # [amount of units, sequence no, weight per unit (kg)].
    unit_dict = {'45G1': [bool_dict['count_40hc'], IMAGE_40HC, data['count_40hc'], 1, "45G1", data['weight_40hc']],
                '42G1': [bool_dict['count_40dv'], IMAGE_40DV, data['count_40dv'], 2, "42G1", data['weight_40dv']],
                '22G1': [bool_dict['count_20dv'], IMAGE_20DV, data['count_20dv'], 3, "22G1", data['weight_20dv']]}
